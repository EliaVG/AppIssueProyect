...
class IssueService{
    // GETS (Read)
    // Traer todas las issues
    getAllIssues(appId){
        return axios.get(`${API_URL}/apps?filters[id][$eq]=${appId}&fields[0]=id&populate=issues`)
    }
    ...
    addNewIssue(id, issues){
        return axios.put(`${API_URL}/apps/${id}`, {data:{issues:issues}})
    }
}
